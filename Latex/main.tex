...
\begin{table}[H]
    \centering
        ...
        \hline
        \textbf{Diffusive flux} & -0.3619 W & -0.1986 W & 9.6817 W & 9.6817 W \\ 
        \hline
        \textbf{Convective flux} & 4.9075 W & -23.8063 W & 0 & 0 \\ 
        \hline
        \end{tabular}
...
The global thermal energy balance, considering all the contributions, is -0.5476 W. This value is very close to the ideal value of zero. The small, non-zero result is an expected and unavoidable consequence of numerical simulation, it stems from the discretization error, as the Finite Volume Method solves a discretized approximation of the continuous governing energy equation.

\bigskip

\textbf{22. Non uniform wall boundary conditions.}
We will now investigate the effect of non uniform wall boundary conditions, to simulate the effect of a localized heating. For both the upper and lower wall we set a positive heat flux $q_{wall} = k\frac{\partial T}{\partial n}$ in the region $ 2 \leq x \leq 5$ and adiabatic wall condition ($k\frac{\partial T}{\partial n}=0$) everywhere else. The goal is to find values of $q_{wall}$ required to reach an average outlet temperature of 60$^\circ$C, 70$^\circ$C and 80$^\circ$C. Running the proposed simulation setup for different values of $q_{wall}$ ranging from 0 to 10 we get the plot shown in \ref{outTemp_wrt_qwall_mixedNeu} .
Observing that the trend is perfectly linear we find some proposed $q_{wall}$ values by inverting the equation and check that they provide the expected temperature outpus by rerunning the simulation. Indeed, average outlet temperatures resulting from proposed $q_{wall}$ match the expected ones. Our findings are summarised in Table \ref{non_uniform_boundary_condition}.

\begin{table}[H]
    \centering
        \begin{tabular}{|c|c|} 
        \hline
        \textbf{Outlet temperature} & $\mathbf{q_{wall}}$ \\ 
        \hline
        60$^\circ$C & 1.516750  \\ 
        \hline
        70$^\circ$C & 3.033500  \\ 
        ...
        \hline
        \end{tabular}
    \caption{Values of $q_{wall}$ needed to reach the specified outlet temperature.}
    \label{non_uniform_boundary_condition}
\end{table}


\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Latex/MATLABfigures/22_1.png} 
    \caption{Average outlet temperature resulting from simulations run applying mixed Neumann condition using different values of $q_{wall}$ }
    \label{outTemp_wrt_qwall_mixedNeu}
\end{figure} 
