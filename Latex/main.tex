...
\begin{figure}[t!]
\centering
\includegraphics[width=60mm]{logo-modified.png}
\end{figure}
...
The 2D plot of the solution $T(x,y)$ can be seen in Figure \ref{temperature_field}. The outlet temperature is shown in Figure \ref{outlet_temperature}, while in Figure \ref{cent_mean_temp} the centerline temperature and the weighted mean temperature are plotted together with the entry length ($x_e = 2.449$). Note that the weighted mean temperature, defined as $T_{mean}(x) = \dfrac{\int_{0}^{H}u_x(y)T(x,y)dy}{\int_{0}^{H}u_x(y)dy}$, is computed numerically, summing up all the contributions over the spanwise direction, in order to approximate the two integrals.
...
Consider the Nusselt Number defined as $Nu = \frac{\partial T / \partial n}{\Delta T / (2H)}$ where $\Delta T = T_{wall}\,-T_{mean}$. For laminar flows in plane channels, as shown by \cite{nusselt}, Nu(x) decreases in the entrance region but, far enough downstream of the inlet, approaches a constant value independent of the Peclet number, this behavior can be observed in Figure \ref{nusselt}. However, contrary to theoretical expectations, the figure also shows a sudden increase in $Nu(x)$ near the outlet. This unexpected trend a numerical artifact, not a physical phenomenon, and is caused by a conflict between the physics of the flow and the Neumann boundary condition. Indeed, a Neumann null outlet condition means that the temperature profile is no longer changing in the direction of the flow. Apparently, this is pacific since it's also what we expect from the definition of \textit{fully developed} flow. In reality, there is still heat transfer occurring at the outlet, full development is just an approximation. This means that the solver has to forcibly bend the temperature profile in the last few cells of the mesh to satisfy the artificial Neumann condition. This unnatural distortion pollutes the solution upstream of the outlet and ultimately the Nusselt number which is calculated using the temperature gradient at the wall and the velocity-averaged mean fluid temperature. Because the solver is distorting the entire temperature field in the final cells, the calculated wall gradient is smaller than what it should be, leading to the artificial spike in the $\mathrm{Nu}$ value we observe.
