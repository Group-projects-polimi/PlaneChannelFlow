...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Latex/MATLABfigures/17_1.png} 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Latex/MATLABfigures/17_2.png} 
...
We now want to impose a Neumann condition \(k \cdot \partial T/\partial x = q_\text{wall}\) on the lower (\(i=n_y\), \(1 \leq j \leq n_x\)) and upper (\(i = 1\), \(1 \leq j \leq n_x\)) wall CVs.

leads to $a_E = 0$. Numerically
\begin{align*}
&A(n, n) = a_P, \quad A(n, n+1) = 0, \quad A(n, n-1) = -a_W, \quad A(n, n+n_x) = -a_N, \\
&A(n, n-n_x) = -a_S, \quad b(n) = 0, \quad \text{where } n = (i-1)n_x + n_x
\end{align*}
